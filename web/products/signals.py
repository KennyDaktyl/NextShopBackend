...
from web.models.products import Product, ProductReview, ProductVariant
...
def revalidate_product_cache_variant(sender, instance, **kwargs):
    ...
    try:
        ...
        response.raise_for_status()
        logger.info(
            f"Successfully revalidated cache for review product {instance.product.slug} and tags {tags}"
        )
    except requests.exceptions.RequestException as e:
        logger.error(
            f"Error revalidating cache for review product {instance.product.id}: {e}"
        )