    ...
    def handle(self, *args, **options):
        ...
        for index, row in df.iterrows():
            ...
            user, created = User.objects.get_or_create(
                ...
                last_name=last_name,
                defaults={'password': row["password"]}  # Ustawiamy hasło tylko dla nowych użytkowników
            )
            ...
            profile, created = Profile.objects.get_or_create(
                ...
                defaults={
                    ...
                    'city': row["city"],
                    'postal_code': row["post_code"].strip().replace(" ", "").replace("-", "")
                }
            )
            ...
            if make_invoice:
                ...
                profile.nip = row["nip"].strip().replace("-", "").replace(" ", "")
                ...
                invoice_postal_code = row["post_code"].strip().replace(" ", "").replace("-", "")
                if len(invoice_postal_code) > 6:
                    self.stderr.write(f"Error: Invoice postal code too long for user {user.username}: {invoice_postal_code}")
                else:
                    profile.invoice_postal_code = invoice_postal_code
                
                profile.save()

            self.stdout.write(f"Profile {user.username} created with profile {profile.make_invoice}")
