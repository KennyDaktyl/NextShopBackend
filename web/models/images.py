...
from serwiswrybnej.settings import MEDIA_ROOT
...
def generate_thumbnails(instance, main, order, relation_name, original_image, file_name=None):
    if not file_name:
...
class Photo(models.Model):
    ...
    variant = models.ForeignKey("ProductVariant", on_delete=models.CASCADE, related_name='photos', null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk:
            ...
            if old_photo.name != self.name:
                ...
                self.thumbnails = generate_thumbnails(instance, False, self.order, instance_name, self.image, self.slug)
        else:
...
class Thumbnail(models.Model):
    ...
    order = models.IntegerField(default=1)
    photo = models.ForeignKey("Photo", on_delete=models.CASCADE, related_name='photo_thumbnails', null=True, blank=True)
    product = models.ForeignKey("Product", on_delete=models.CASCADE, related_name='product_thumbnails', null=True, blank=True)
    product_variant = models.ForeignKey("ProductVariant", on_delete=models.CASCADE, related_name='variant_thumbnails', null=True, blank=True)
    category = models.ForeignKey("Category", on_delete=models.CASCADE, related_name='category_thumbnails', null=True, blank=True)
    width = models.IntegerField(verbose_name="Szerokość") 
    height = models.IntegerField(verbose_name="Wysokość")
    width_expected = models.IntegerField(verbose_name="Szerokość", null=True, blank=True)
    height_expected = models.IntegerField(verbose_name="Wysokość", null=True, blank=True)
...
def create_thumbnail(relation, relation_name, size, main, order, original_image, file_name):
    thumbnail = Thumbnail()
    
    if not order:
        filter_kwargs = {relation_name: relation}
        order = Thumbnail.objects.filter(width=size[0], height=size[1], **filter_kwargs).count() + 1
    ...
    if relation_name == 'product':
        ...
        raise ValueError("Relation must be 'product', 'variant', 'category' or 'photo'.")
    ...
    filename = f"{file_name.replace(' ', '_').lower()}_{size[0]}x{size[1]}.webp"
    thumbnail.main = main
